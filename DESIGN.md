...
이 시스템 고유의 데이터 표현: **차트가 아니라 문장이 지표를 말하고, 위치가 진척을 말한다.**
- **오늘의 한 걸음:** 앰버 라벨 + 22px 문장형 히어로. 원자적이고 실행 가능한 한 걸음(무대에 가장 가까운 원고 "개작을 이어서") — 백로그 숫자는 절대 히어로가 아니다.
- **두 세계의 다리:** 히어로 아래 한 문장 — "『경험의멸종』에서 자란 『리뷰를 먼저 읽은 장소』가 무대로 향합니다." 독서(입력)와 창작(출력)이 한 줄에서 만나는 이 제품의 진짜 시그니처. sources 역추적으로 자동 생성.
- **서가 문장:** "지금까지 N편을 읽었고, 서가에는 새 원고 M편이 쌓여 있습니다." — 미채점은 결손이 아니라 쌓여가는 원고 더미(자산). 상세는 `<details>` 서랍.
- **무대까지의 거리 (선반):** 파이프라인을 칸반 4열이 아니라 **무대에 가까운 순으로 쌓인 가로 선반**으로 그린다(개작=무대 코앞 → 조립 → 대기 → 설계). 무대 근처 선반만 크고 앰버 ★를 받으며, 먼 선반은 잔잔한 나열 + 서랍. 칸반은 안티레퍼런스(생산성 앱)의 문법이라 쓰지 않는다.
- **흐름 막대:** `width: N%` 단일 div, 비상구 초록. 책→창작 파생량.
- **금지된 문법:** 커버리지 도넛, 배치별 스택 바, 칸반 보드 — 처리량 회고·생산성 압박의 문법은 전부 제거되었다(2026-07-04 크리틱 2회차). 사용자가 물을 수 없는 숫자(배치 번호)는 그리지 않는다.
